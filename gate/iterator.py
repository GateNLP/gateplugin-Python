		...
		if line:
			...
			if "command" in input_json:
				...
				try:
					...
					scriptParams = scriptParams if scriptParams else {}
					...
					inputAS = scriptParams["inputAS"]
					outputAS = scriptParams["outputAS"]

					yield document, scriptParams
					print json.dumps(document.logger)
				except InvalidOffsetException as e:
					print >> sys.stderr, "InvalidOffsetException prevented reading a document"
					print json.dumps([])
				sys.stdout.flush()
